...
displayInline: function (messages) {
    if ( messages!=null && messages.length>0 ) {
        ...
        for (var i=0; i<messages.length; i++) {
            ...
            //modify for cakephp start
            try {
                var elInput = document.getElementsByName(elName);
                var cakeErr = elInput[0].nextSibling;
                if (cakeErr != null) {
                    if (cakeErr.className.indexOf('error-message',0) != -1) {
                        return false;
                    }
                }
            } catch(e) {}
          //modify for cakephp end
...
getDefaultMessage: function (el, nameDisplayed, ruleName, comparisonValue) {
    ...
    if (ruleName=='required') {
        msg = yav_config.REQUIRED_MSG;
    } else if (ruleName=='minlength') {
        msg = yav_config.MINLENGTH_MSG.replace('{2}', comparisonValue);
    } else if (ruleName=='maxlength') {
        msg = yav_config.MAXLENGTH_MSG.replace('{2}', comparisonValue);
    } else if (ruleName=='numrange') {
        msg = yav_config.NUMRANGE_MSG.replace('{2}', comparisonValue);
    } else if (ruleName=='date') {
        msg = yav_config.DATE_MSG;
    } else if (ruleName=='numeric') {
        msg = yav_config.NUMERIC_MSG;
    } else if (ruleName=='integer') {
        msg = yav_config.INTEGER_MSG;
    } else if (ruleName=='double') {
        msg = yav_config.DOUBLE_MSG;
    } else if (ruleName=='equal') {
        msg = yav_config.EQUAL_MSG.replace('{2}', yav.getComparisonDisplayed(comparisonValue));
    } else if (ruleName=='notequal') {
        msg = yav_config.NOTEQUAL_MSG.replace('{2}', yav.getComparisonDisplayed(comparisonValue));
    } else if (ruleName=='alphabetic') {
        msg = yav_config.ALPHABETIC_MSG;
    } else if (ruleName=='alphanumeric') {
        msg = yav_config.ALPHANUMERIC_MSG;
    } else if (ruleName=='alnumhyphen') {
        msg = yav_config.ALNUMHYPHEN_MSG;
    } else if (ruleName=='alnumhyphenat') {
        msg = yav_config.ALNUMHYPHENAT_MSG;
    } else if (ruleName=='alphaspace') {
        msg = yav_config.ALPHASPACE_MSG;
    } else if (ruleName=='email') {
        msg = yav_config.EMAIL_MSG;
    } else if (ruleName=='regexp') {
        msg = yav_config.REGEXP_MSG.replace('{2}', comparisonValue);
    } else if (ruleName=='date_lt') {
        msg = yav_config.DATE_LT_MSG.replace('{2}', yav.getComparisonDisplayed(comparisonValue));
    } else if (ruleName=='date_le') {
        msg = yav_config.DATE_LE_MSG.replace('{2}', yav.getComparisonDisplayed(comparisonValue));
    } else if (ruleName=='empty') {
        msg = yav_config.EMPTY_MSG;
    }
    return msg;
},
