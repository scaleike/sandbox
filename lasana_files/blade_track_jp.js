(function(){
	...
	function sp_track(co_account_id, group_id, country_id, version) {

		var init = function () {
			var microadDevice = new MicroadBladeDevice(navigator.userAgent);
			if (microadDevice) {
				new BladeSponsorAccess(co_account_id, group_id, microadDevice, track_host(), country_id, version);
			}
		};
	...
	function bl_track(co_account_id, group_id, country_id, version) {
		if(/(iPhone|iPod|iPad|Android)/.test(navigator.userAgent)){
			sp_track(co_account_id, group_id, country_id, version);
		} else {
